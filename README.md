...
You need:
    ...
    Windows 10/11
    Python 3.13 — grab it from python.org/downloads, NOT bundled in this repo (nobody needs a 25MB installer.exe sitting in their git history for the rest of time). On the install screen, check "Add python.exe to PATH" — this is the step everyone skips and then is sad about.
    Either Ollama (free, local, runs entirely offline, your secrets stay yours) or an API key for Claude/GPT/Gemini if you want to phone a friend with more VRAM than you.
